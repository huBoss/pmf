...
namespace PMF
{
    public class ChartStyle2D
    {
        ...
        public ChartStyle2D(PMF fm1)
        {
            frmChat = fm1;

            Rectangle rect = frmChat.ClientRectangle;
        ...
        private void AddLabels(Graphics g, ChartStyle cs3d)
        {
            ...
            for (float fX = cs3d.XMin; fX <= cs3d.XMax; fX += cs3d.XTick)
            {
                ...
                g.DrawString(fX.ToString(), cs3d.TickFont, aBrush,
                    new PointF(yAxisPoint.X + sizeXTick.Width / 2 + frmChat.PlotPanel.Left,
                    yAxisPoint.Y + 4f + frmChat.PlotPanel.Top), sFormat);
            }
            ...
            for (float fY = cs3d.YMin; fY <= cs3d.YMax; fY += cs3d.YTick)
            {
                ...
                g.DrawString(fY.ToString(), cs3d.TickFont, aBrush,
                    new PointF(xAxisPoint.X - 3f + frmChat.PlotPanel.Left,
                    xAxisPoint.Y - tickFontSize.Height / 2 + frmChat.PlotPanel.Top), sFormat);
            }
            ...
            g.DrawString(cs3d.XLabel, cs3d.LabelFont, aBrush,
                new Point(frmChat.PlotPanel.Left + frmChat.PlotPanel.Width / 2 -
            ...
            g.TranslateTransform(xOffset, yOffset + titleFontSize.Height
                + yOffset / 3 + frmChat.PlotPanel.Height / 2);
        ...
        public void SetPlotArea(Graphics g, ChartStyle cs3d)
        {
            ...
            frmChat.PlotPanel.Left = plotX;
            frmChat.PlotPanel.Top = plotY;
            if (cs3d.IsColorBar)
                frmChat.PlotPanel.Width = 25 * plotWidth / 30;
            else
                frmChat.PlotPanel.Width = plotWidth;
            frmChat.PlotPanel.Height = plotHeight;
            AddLabels(g, cs3d);
        }
        ...
        public PointF Point2D(PointF pt, ChartStyle cs3d)
        {
            ...
            aPoint.X = (pt.X - cs3d.XMin) *
                frmChat.PlotPanel.Width / (cs3d.XMax - cs3d.XMin);
            aPoint.Y = frmChat.PlotPanel.Height - (pt.Y - cs3d.YMin) *
                frmChat.PlotPanel.Height / (cs3d.YMax - cs3d.YMin);
            return aPoint;
        }
    }
}
